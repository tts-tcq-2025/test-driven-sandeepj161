...
TEST_F(StringCalculatorTest, TwoNumbersSum) {
  EXPECT_EQ(calc.add("1,2"), 3);
}
TEST_F(StringCalculatorTest, MultipleNumbers) {
...
TEST_F(StringCalculatorTest, CustomDelimiterWithSpaces) {
  EXPECT_EQ(calc.add("//[ab cd]\n1ab cd2ab cd3"), 6);
}
TEST_F(StringCalculatorTest, MixedDefaultDelimiters) {
  EXPECT_EQ(calc.add("4\n5,6"), 15);
}
TEST_F(StringCalculatorTest, CustomHeaderWithCommasStillWorks) {
  EXPECT_EQ(calc.add("//;\n1;2,3"), 6);
}
...
// --- >1000 rule ---
TEST_F(StringCalculatorTest, NumbersAbove1000AreIgnored) {
...
TEST_F(StringCalculatorTest, MissingNewlineAfterHeaderReturnsZero) {
  EXPECT_EQ(calc.add("//;1;2"), 0);
}
TEST_F(StringCalculatorTest, EmptyDelimiterBracketsFallbackToDefault) {
  EXPECT_EQ(calc.add("//[]\n1,2"), 3);
}
TEST_F(StringCalculatorTest, MultiCharacterDelimiterWithoutBracketsUndefined) {
  EXPECT_EQ(calc.add("//***\n1***2***3"), 0);
}
